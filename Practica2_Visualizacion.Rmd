...
#Creacion del objeto ggplot
grafico1 <- ggplot(ds, aes(x = MinTemp, y = MaxTemp))
grafico1

#Añadimos una capa para definir un objeto geometrico para construir un grafico de dispersion, en este caso puntos 
grafico_1 <- grafico1 + geom_point()
grafico_1

#Visualizacion de las variables en los ejes de la grafica y el color
grafico2 <- ggplot(ds, aes(x = MinTemp, y = MaxTemp, colour = RainTomorrow)) + geom_point()
grafico2

#Añadimos un titulo al grafico
grafico3 <- grafico2 + ggtitle("Grafico de dispersion: temperaturas maximas y minimas")
grafico3

#Almacenamos en formato pdf el grafico resultante (Ejercicio1.pdf)
...
grafico1 <- ggplot(ds, aes(x = Temp9am, y = Pressure9am))
grafico1

#Incluimos informacion categorica con la forma del punto
grafico2 <- ggplot(ds, aes(x = Temp9am, y = Pressure9am, shape = RainToday)) + geom_point()
grafico2

#Incluimos informacion categorica sobre el color de los puntos
grafico3 <- ggplot(ds, aes(x = Temp9am, y = Pressure9am, shape = RainToday, colour = RainToday)) + geom_point()
grafico3
#Claramente vemos que con la representacion a color se observa mejor la informacion

#Añadimos un titulo
grafico4 <- grafico3 + ggtitle("Grafica de dispersion: presion frente a temperatura")
grafico4

#Almacenamos en formato pdf el grafico resultante (Ejercicio2.pdf)
...
#Visualizacion de las variables contenidas en el dataset
str(iris.tidy)

#Creamos un objeto ggplot
iris.grafico1 <- ggplot(iris.tidy, aes(x = Species, y = Value))
iris.grafico1

#Añadimos una capa en la que definimos el objeto geometrico necesario para la construccion del grafico de dispersion
iris.grafico_1 <- iris.grafico1 + geom_point()
iris.grafico_1

#Modificamos el aesthetics del grafico anterior para representar el color del punto correspondiente
iris.grafico2 <- ggplot(iris.tidy, aes(x = Species, y = Value, colour = Part)) + geom_point()
iris.grafico2

#Incluimos informacion creando dos graficos de dispersion
iris.grafico3 <- ggplot(iris.tidy, aes(x = Species, y = Value, colour = Part)) + geom_point() + facet_grid(Measure~.)
iris.grafico3
#Cambiamos la dispersion de los graficos en columnas
iris.grafico4 <- ggplot(iris.tidy, aes(x = Species, y = Value, colour = Part)) + geom_point() + facet_grid(.~Measure)
iris.grafico4
#Observamos que la dispersion en columnas nos permite comparar mejor los valores de dichas variables

#Hacemos un cambio de capas
iris.grafico5 <- ggplot(iris.tidy, aes(x = Species, y = Value, colour = Part)) + geom_jitter() + facet_grid(.~Measure)
iris.grafico5
#La mayor diferencia que podemos observar es que con geom_point() los datos aparecen representados mas unidos, mientras que con geom_jitter() estan mas dispersos

#Almacenamos en formato pdf el grafico resultante (Ejercicio3.pdf)
...
#Examinamos las variables del dataset
str(DatosEjer4)

#Representamos un grafico de puntos
grafico1 <- ggplot(data = DatosEjer4, aes(x = Fecha, y = Ventas, colour = Fabricante)) + geom_point()
grafico1
#Como vemos este grafico no es valido para evaluar la evolucion de las ventas de cada fabricantes, nos faltaria unir los puntos para tener una mejor representacion

#Añadimos las lineas que unen cada punto
grafico2 <- ggplot(data = DatosEjer4, aes(x = Fecha, y = Ventas, colour = Fabricante)) + geom_point() + geom_line()
grafico2

#Trazamos la tendencia de cada fabricante con un tipo de linea diferente
grafico3 <- ggplot(data = DatosEjer4, aes(x = Fecha, y = Ventas, colour = Fabricante, linetype = Fabricante)) + geom_point() + geom_line()
grafico3

#Añadimos un titulo al grafico
grafico4 <- grafico3 + ggtitle("Ventas mensuales de diferentes compañias tecnologicas")
grafico4

#Almacenamos en pdf el grafico resultante (Ejercicio4.pdf)
...
#Representacion del grafico de puntos
grafico1 <- ggplot(data = Experimento, aes(x = Dosis, y = Altura, shape = Complemento, linetype = Complemento, colour = Complemento)) + geom_point() + geom_line()
grafico1

#Añadimos al grafico anterior barras de error en cada punto
grafico2 <- ggplot(data = Experimento, aes(x = Dosis, y = Altura, shape = Complemento, linetype = Complemento, colour = Complemento)) + geom_point() + geom_line() + geom_errorbar(aes(ymin = Altura-Error_Altura, ymax = Altura+Error_Altura), width = .1, position = position_dodge(0.05))
grafico2

#Eliminamos el tipo le linea y punto, con colores rojos
...
#Almacenamos en formato pdf el grafico resultante (Ejercicio5.pdf)
...
#Creacion del grafico en el que se encuentren los boxplots de la temperatura minima para cada año
grafico1 <- ggplot(ds, aes(x = Year, y = MinTemp)) +  geom_boxplot()
grafico1

#Creacion del grafico donde esten los graficos de violin de la temperatura maxima para cada año
grafico2 <- ggplot(ds, aes(x = Year, y = MaxTemp, fill = Year)) + geom_violin()
#Añadimos titulo
grafico_2 <- grafico2 + labs(title = "Distribucion de temperaturas maximas por año", x = "año", y = "Temperatura(ºC)")
grafico_2

#Eliminamos la leyenda y colocamos los años en el eje x a 45 grados de inclinacion
grafico3 <- grafico_2 + theme(axis.text.x = element_text(angle = 45), legend.position = "none")
grafico3

#Almacenamos en formato pdf el grafico resultante (Ejercicio6.pdf)
...
df <- read.table("http://assets.datacamp.com/blog_assets/chol.txt", header = TRUE)

#Representamos el histograma
grafico1 <- ggplot(df, aes(x = HEIGHT)) + geom_histogram()
grafico1
...
#Ajustamos el rango de representacion y conseguimos que el histograma tenga 20 bins
grafico3 <- ggplot(df, aes(x = HEIGHT)) + geom_histogram(breaks = seq(155, 195, by = 2), bins = 20) + labs(title = "Histograma de la variable altura", x = "Altura (cm)", y = "Frecuencia")
grafico3

#Cambiamos el color de las barras
grafico4 <- ggplot(df, aes(x = HEIGHT)) + geom_histogram(breaks = seq(155, 195, by = 2), bins = 20, fill = "Blue", colour = "Red", alpha = .5) + labs(title = "Histograma de la variable altura", x = "Altura (cm)", y = "Frecuencia")
grafico4

#Añadimos curva de distribucion
grafico5 <- ggplot(df, aes(x = HEIGHT)) + geom_histogram(aes(y = ..density..), breaks = seq(155, 195, by = 2), bins = 20, fill = "Blue", colour = "Red", alpha = .5) + labs(title = "Histograma de la variable altura", x = "Altura (cm)", y = "Frecuencia") + geom_density(col = "Green")
grafico5

#Representamos el histograma anterior para cada valor de "SMOKE"
grafico6 <- ggplot(df, aes(x = HEIGHT)) + geom_histogram(aes(y = ..density..), breaks = seq(155, 195, by = 2), bins = 20, fill = "Blue", colour = "Red", alpha = .5) + labs(title = "Histograma de la variable altura", x = "Altura (cm)", y = "Frecuencia") + geom_density(col = "Green") + facet_grid(.~SMOKE)
grafico6

#Almacenar en pdf el fichero representado (Ejercicio7.pdf)
